...
<div class="w-80 mx-auto flex justify-center items-center h-screen">
  <FollowPointerCard title="Sikandar Bhide">
    ...
    >
      <div
        class="w-full aspect-w-16 aspect-h-10 rounded-tr-lg bg-black rounded-tl-lg overflow-hidden xl:aspect-w-16 xl:aspect-h-10 relative"
      >
        <img
          src={blogContent.image}
          alt="thumbnail"
          class="group-hover:scale-95 group-hover:rounded-2xl transform object-cover transition duration-200 w-full h-full"
        />
      </div>
      <div class="px-4 pb-2">
        <h2 class="font-bold my-4 text-lg text-black">Coding is fun</h2>
        ...
        <div class="flex flex-row justify-between items-center mt-2">
          ...
          <div
            class="relative z-10 px-6 py-2 bg-black text-white font-bold rounded-xl block text-xs"
          >
            Read More
          </div>
        </div>
