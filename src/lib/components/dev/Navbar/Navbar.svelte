...
>
            ...
            {#each navs as item}
              <a
                ...
                item.link
                  ...
                  : 'text-primary/70'} {item.name === 'Luxe'
                  ? 'text-fuchsia-500 font-normal bg-fuchsia-100 dark:bg-fuchsia-900/70 hover:text-fuchsia-600'
                  : ''}">{item.name}</a
              >
            {/each}
          </div>
  ...
  {#key mobileMenu}
    <div
      class="{mobileMenu ? 'flex' : 'hidden'} sm:hidden w-full"
      id="mobile-menu"
    >
